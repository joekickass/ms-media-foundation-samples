...
Finally, the generated header file (__%(RootNamespace)_h.h__) needs to be referenced in the class definition file (_*.cpp_) as shown in the generic example below. Notice the __InspectableClass__ using a string definition from the header, and the __ActivatableClass__ section.
